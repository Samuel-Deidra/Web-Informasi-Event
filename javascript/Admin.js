...
async function handleAddEventSubmit(e) {
    ...
    if (!namaEvent || !tanggalEvent || !tanggalPendaftaranAwal || !tanggalPendaftaranAkhir || !jamEvent || !tautan || !deskripsi) {
        showErrorMessage('Mohon lengkapi semua field yang wajib diisi, termasuk Link dan Deskripsi');
        isProcessing = false;
        return;
    }
    ...
    // Validasi format jam
    if (!isValidTime(jamEvent)) {
        showErrorMessage('Format jam event tidak valid. Gunakan format HH:MM (contoh: 14:30)');
        isProcessing = false;
        return;
    }
