...
function populateEditForm(event) {
  ...
  document.getElementById("editTanggalEventAkhir").value =
  ...
  if (
    ...
    const fpReg = document.getElementById(
      "editTanggalPendaftaranRange"
    )._flatpickr;
    if (
      fpReg &&
      event.tanggal_pendaftaran_awal &&
      ...
      fpReg.setDate(
        [event.tanggal_pendaftaran_awal, event.tanggal_pendaftaran_akhir],
        true
      );
    }
  }
...
function setupFormHandlers() {
  ...
  if (typeof flatpickr !== "undefined") {
    const regRange = document.getElementById("tanggalPendaftaranRange");
    const regStartHidden = document.getElementById("tanggalPendaftaranAwal");
    const regEndHidden = document.getElementById("tanggalPendaftaranAkhir");
    if (regRange)
      flatpickr(regRange, {
        mode: "range",
        dateFormat: "Y-m-d",
        ...
        onChange: function (selectedDates) {
          if (selectedDates.length >= 1)
            regStartHidden.value = formatYMD(selectedDates[0]);
          if (selectedDates.length >= 2)
            regEndHidden.value = formatYMD(selectedDates[1]);
        },
      });
    const eventRange = document.getElementById("tanggalEventRange");
    const eventHidden = document.getElementById("tanggalEvent");
    const eventEndHidden = document.getElementById("tanggalEventAkhir");
    if (eventRange)
      flatpickr(eventRange, {
        mode: "range",
        dateFormat: "Y-m-d",
        locale: "id",
        onChange: function (selectedDates) {
          if (selectedDates.length >= 1)
            eventHidden.value = formatYMD(selectedDates[0]);
          if (selectedDates.length >= 2)
            eventEndHidden.value = formatYMD(selectedDates[1]);
          else eventEndHidden.value = "";
        },
      });
    const editRegRange = document.getElementById("editTanggalPendaftaranRange");
    const editRegStartHidden = document.getElementById(
      "editTanggalPendaftaranAwal"
    );
    const editRegEndHidden = document.getElementById(
      "editTanggalPendaftaranAkhir"
    );
    if (editRegRange)
      flatpickr(editRegRange, {
        mode: "range",
        dateFormat: "Y-m-d",
        locale: "id",
        onChange: function (selectedDates) {
          if (selectedDates.length >= 1)
            editRegStartHidden.value = formatYMD(selectedDates[0]);
          if (selectedDates.length >= 2)
            editRegEndHidden.value = formatYMD(selectedDates[1]);
        },
      });
    const editEventRange = document.getElementById("editTanggalEventRange");
    const editEventHidden = document.getElementById("editTanggalEvent");
    const editEventEndHidden = document.getElementById("editTanggalEventAkhir");
    if (editEventRange)
      flatpickr(editEventRange, {
        mode: "range",
        dateFormat: "Y-m-d",
        locale: "id",
        onChange: function (selectedDates) {
          if (selectedDates.length >= 1)
            editEventHidden.value = formatYMD(selectedDates[0]);
          if (selectedDates.length >= 2)
            editEventEndHidden.value = formatYMD(selectedDates[1]);
          else editEventEndHidden.value = "";
        },
      });
  }
...
async function handleAddEventSubmit(e) {
  ...
  if (
    ...
    showErrorMessage(
      "Mohon lengkapi semua field yang wajib diisi (Link bersifat opsional)"
    );
    isProcessing = false;
    return;
  }
  ...
  if (biaya && (isNaN(parseFloat(biaya)) || parseFloat(biaya) < 0)) {
    showErrorMessage(
      "Biaya harus berupa angka yang valid dan tidak boleh negatif"
    );
    isProcessing = false;
    return;
  }
  ...
  formData.append(
    "tanggal_event_akhir",
    document.getElementById("tanggalEventAkhir").value
  );
  ...
  try {
    ...
    if (result.success) {
      ...
      addModal._element.addEventListener(
        "hidden.bs.modal",
        function () {
          e.target.reset();
          document.getElementById("formStep2").classList.add("d-none");
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent =
        submitBtn.getAttribute("data-original-text") || "Simpan";
    }
    isProcessing = false;
  }
}
...
async function handleEditEventSubmit(e) {
  ...
  try {
    ...
    if (result.success) {
      ...
      editModal._element.addEventListener(
        "hidden.bs.modal",
        function () {
          e.target.reset();
          document.getElementById("editFormStep2").classList.add("d-none");
    ...
    if (submitBtn) {
      submitBtn.disabled = false;
      submitBtn.textContent =
        submitBtn.getAttribute("data-original-text") || "Perbarui";
    }
    isProcessing = false;
  }
}
...
function showDeleteConfirmation(eventId, eventName, row) {
  ...
  if (isHistoryPage) {
    ...
    modalTitle.textContent = "Hapus Event?";
    modalText.textContent = `Apakah Anda yakin ingin menghapus event "${eventName}"? Event akan dipindahkan ke history.`;
...
async function handleDeleteConfirm() {
  ...
  try {
    ...
    const url = currentRowToDelete.isHistoryPage
      ? `${API_URL}?id=${currentRowToDelete.eventId}&permanent=1`
...
function createEventRow(event, isHistory = false) {
  ...
  const formattedEventStart = formatDateFromString(event.tanggal_event);
...
function createDetailsRow(event) {
  ...
  const formattedRegStart = formatDateFromString(
    event.tanggal_pendaftaran_awal
  );
  const formattedRegEnd = formatDateFromString(event.tanggal_pendaftaran_akhir);
  const formattedEventStart = formatDateFromString(event.tanggal_event);
  const formattedEventEnd = event.tanggal_event_akhir
    ? formatDateFromString(event.tanggal_event_akhir)
    : "";
...
function filterTable() {
  ...
  const filteredData = activeData.filter((event) => {
    ...
    const matchKategori =
      !kategori ||
      (event.kategori &&
        event.kategori.toLowerCase() === kategori.toLowerCase());
    const matchStatus =
      !status ||
...
function showSuccessToast(title, message) {
  ...
  toast.innerHTML = `<div class="toast-icon"><i class="fa-solid fa-check"></i></div><div class="toast-content"><div class="toast-title">${title}</div><div class="toast-message">${message}</div></div>`;
...
function showErrorMessage(message) {
  ...
  const toast = document.createElement("div");
  toast.className = "success-toast";
  toast.innerHTML = `<div class="toast-icon" style="background: #fee2e2;"><i class="fa-solid fa-exclamation-triangle" style="color: #dc2626;"></i></div><div class="toast-content"><div class="toast-title">Error</div><div class="toast-message">${message}</div></div>`;
  toastContainer.appendChild(toast);
  setTimeout(() => {
    toast.style.animation = "slideIn 0.3s ease-out reverse";
    setTimeout(() => toast.remove(), 300);
